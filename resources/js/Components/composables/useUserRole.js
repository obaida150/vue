...
export function useUserRole() {
    ...
    const fetchUserRole = async () => {
        try {
            ...
            console.log("[v0] Backend response from /api/user/role:", response.data)
            console.log("[v0] role_id from backend:", response.data.role_id)
            console.log("[v0] user_id from backend:", response.data.user_id)
            console.log("[v0] has_hr_permissions from backend:", response.data.has_hr_permissions)
            ...
            console.log("[v0] After setting - currentUserRoleId:", currentUserRoleId.value)
            console.log("[v0] After setting - hasHrPermissions:", hasHrPermissions.value)
            console.log("[v0] After setting - isHrUser:", isHrUser.value)
    ...
    const fetchEmployees = async () => {
        if (currentUserRoleId.value === null) {
            console.log("[v0] Waiting for user role to be loaded...")
            await fetchUserRole()
        }

        console.log(
            "[v0] fetchEmployees called - role_id:",
        ...
        if (currentUserRoleId.value === 1 || currentUserRoleId.value === 2 || hasHrPermissions.value) {
            try {
                console.log("[v0] Admin/HR User - Fetching all employees from /api/users...")
                ...
                console.log("[v0] All employees loaded for HR:", response.data.length)
        ...
        try {
            console.log("[v0] Team Manager - Fetching employees from /api/users...")
            ...
            console.log("[v0] Employees loaded for Team Manager:", response.data.length)
            employees.value = response.data
        } catch (error) {
            console.error("Fehler beim Laden der Mitarbeiterliste:", error)
            console.error("[v0] Error response:", error.response?.data)
